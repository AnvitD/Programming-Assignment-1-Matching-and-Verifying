def gale_shapley(n, hos_prefs, stu_prefs):
    if n == 0 or not hos_prefs or not stu_prefs:
        raise ValueError("Incomplete Files or no input provided.")

    if len(hos_prefs) != n:
        raise ValueError("Missing hospital preference lists.")
    if len(stu_prefs) != n:
        raise ValueError("Missing student preference lists.")

    for i, prefs in enumerate(hos_prefs, 1):
        if len(prefs) != n:
            raise ValueError(f"Hospital {i} does not have {n} preferences.")
    ...
    for i, prefs in enumerate(stu_prefs, 1):
        if len(prefs) != n:
            raise ValueError(f"Student {i} does not have {n} preferences.")
        if sorted(prefs) != list(range(1, n + 1)):
